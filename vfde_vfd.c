...
typedef struct {
    ...
    hal_u32_t    probe_tool_no;    // Probe tool number
    hal_u32_t   *current_tool_no;  // Current tool number
...
typedef struct params {
    ...
    int probe_tool_no; // probe tool number
...
retry:
    ...
    if (*(haldata->current_tool_no) == haldata->probe_tool_no) {
        return 0;
    }
...
int hal_setup(int id, haldata_t *h, const char *name)
{
    ...
    PIN(hal_pin_u32_newf(HAL_IN, &(h->current_tool_no), id, "%s.current-tool-no", name));
    ...
    PIN(hal_param_u32_newf(HAL_RW, &(h->probe_tool_no), id, "%s.probe_tool_no", name));
...
int set_defaults(param_pointer p)
{
    ...
    *(h->current_tool_no) = 0;
    ...
    h->probe_tool_no = p->probe_tool_no;
